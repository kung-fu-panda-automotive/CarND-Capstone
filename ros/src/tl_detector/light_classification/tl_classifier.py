...
class TLClassifier(object):
  ...
  def get_classification(self, image):
    ...
    Args:
        image (cv::Mat): image containing the traffic light. Can be arbitrary size, but expected to
                         have BGR channels, usual OpenCV format.
    ...
    labels, probabilities = self._cnn_classifier.predict(np.array([resized]), batch_size=1)
