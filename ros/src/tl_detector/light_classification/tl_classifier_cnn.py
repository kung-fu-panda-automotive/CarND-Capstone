...
class TLClassifierCNN(object):
    ...
    * load existing model and run inference
    ...
    def _conv2d(self, input_op, input_channels, output_channels, kernel_size, stride_size):
        ...
        weights = tf.Variable(tf.truncated_normal(params, stddev=trunc_normal_stddev),
                              name='weights')
    ...
    def _create_layer_2(self):
        ...
        with tf.name_scope("conv2"):
            ...
            stride_size = 1
            # conv
            conv = self._conv2d(self._conv1, input_channels, output_channels, kernel_size,
                                stride_size)
            # relu
            activations = tf.nn.relu(conv)
            tf.summary.histogram('activations', activations)
    ...
    def train(self,
        ...
        Args:
          ...
          :param max_iterations_without_improvement: number of epochs after which to stop training
        ...
        for epoch_i in range(epochs):
            ...
            batches_pbar = tqdm(range(n_batches),
                                desc='Train Epoch {:>2}/{}'.format(epoch_i + 1, epochs),
                                unit='batches')
            for batch_i in batches_pbar:
                ...
                _, _, summaries = self._session.run(
            ...
            batches_pbar = tqdm(range(n_batches),
                                desc='Accuracy Epoch {:>2}/{}'.format(epoch_i + 1, epochs),
                                unit='batches')
            a = 0.
            l = 0.
            for batch_i in batches_pbar:
            ...
            print 'epoch {}: validation accuracy {}, validation loss {}'.format(epoch_i,
                                                                                validation_accuracy,
                                                                                validation_loss)
            if validation_accuracy > best_validation_accuracy:
                ...
                if epoch_i - last_improvement_epoch >= max_iterations_without_improvement:
                    print 'no validation accuracy improvement over {} epochs. stop'\
                        .format(max_iterations_without_improvement)
                    break  # stop learning
    ...
    def predict(self,
        ...
        Returns:
          ...
          :return predicted_probabilities: numpy array of softmax probabilities vectors, columns in
                                           each vector are ordered as per TLLabelConverter
        """
